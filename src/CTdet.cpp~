...
void CTdet::imageCallback(const sensor_msgs::ImagePtr& imgData) { 
  ...
  cv_bridge::CvImagePtr cv_ptr_blue = nullptr;	
	...
	ofstream myfile_red("crabs_test.csv", std::ios_base::app); 	//For red color
	ofstream myfile_green("plants_test.csv", std::ios_base::app);	//For green color
	ofstream myfile_blue("rocks_test.csv", std::ios_base::app); 	//for blue color
...
//### White Pixel Counter for GREEN color ###
 ...
 cv::Mat partROI_green;	
	...
	if (perc_green >= 0.28)
	{
	    cout << "Percentage of green objects covering surface = " << perc_green << endl;
	    cout << "latitude " << m_gpsPosition.latitude << " longitude " << m_gpsPosition.longitude << endl;
...
myfile_red.close(); //hier csv schließen
std::cerr << "CLOSING CRABS_TEST.CSV" << std::endl;
myfile_green.close(); //hier csv schließen
std::cerr << "CLOSING PLANTS_TEST.CSV" << std::endl;
myfile_blue.close(); //hier csv schließen
std::cerr << "CLOSING ROCKS_TEST.CSV" << std::endl;
