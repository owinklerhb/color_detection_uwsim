...
//RED COLOR
    ...
    Mat1b mask = mask1 | mask2;
	    cv::cvtColor(imgMat, cv_ptr->image, CV_HSV2BGR);
    //imshow("Mask", mask);
    //waitKey();
    //return 0;
	 ...
	 /* Gray scale image */
           cv::Mat filtered_image2;
            cv::cvtColor(imgMat,filtered_image2,CV_BGR2GRAY);

	// threshold image 
	    int global_min_threshold=50; //hier die Untergrenze des Threshold
	    cv::Mat threshold_image;
	    cv::threshold(filtered_image2,imgMat,global_min_threshold,255,cv::THRESH_BINARY); //Obergrenze auf 255 für echtes Weiß
